...
// type
import { ResponsivedLayoutRowProps } from "./types";
// utils
...
const getResponsive = (responsive, flexDirection, display, gap) => {
	...
	resData.forEach((itemSize) => {
		...
		${item.gap === 0 ? "gap:0px;" : getGap(lastDirection, lastGap, lastDisplay)}
	}`;
	});
	return resStyle;
};
...
const getGap = (flexDirection, gap, display) => {
	if (display === "grid") {
		return typeof gap === "string" ? `gap: ${gap};` : `gap: ${gap}px;`;
	} else if (flexDirection === "column") {
		return ` height: 100%; & > *:not(:last-child) {
					margin-right: 0px;
